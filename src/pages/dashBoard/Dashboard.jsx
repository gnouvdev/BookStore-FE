    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                setData({
                    totalBooks: responseData.totalBooks || 0,
                    totalSales: responseData.totalSales || 0,
                    trendingBooks: responseData.trendingBooks || 0,
        ...
        <>
            <section className="grid md:grid-cols-2 xl:grid-cols-4 gap-6">
                ...
                <div className="flex items-center p-8 bg-white shadow rounded-lg">
                    ...
                    <div>
                        <span className="block text-2xl font-bold">{data.totalSales.toLocaleString('vi-VN', { style: 'currency', currency: 'VND' })}</span>
                            ...
                            {data.users.map((user, index) => (
                                <li key={user._id || index} className="flex items-center">
                                    <div className="h-10 w-10 mr-3 bg-gray-100 rounded-full overflow-hidden">
                                        <img 
                                            src={user.avatar || `https://randomuser.me/api/portraits/${index % 2 ? 'women' : 'men'}/${80 - index}.jpg`} 
                                            alt={`${user.name || 'User'}'s profile picture`}
                                        />
                                    </div>
                                    <span className="text-gray-600">{user.name || 'Unknown User'}</span>
                                    <span className="ml-auto font-semibold">{user.averageOrder || 0}</span>
                                </li>
                            ))}
                        </ul>
                    </div>
