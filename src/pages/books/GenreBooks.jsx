...
const EnhancedGenreBooks = () => {
  ...
  // Hàm lấy giá số từ book.price
  const getBookPrice = (book) => {
    if (typeof book.price === "object" && book.price !== null) {
      return typeof book.price.newPrice === "number"
        ? book.price.newPrice
        : typeof book.price.oldPrice === "number"
        ? book.price.oldPrice
        : 0;
    }
  ...
  const filteredBooks = books
    .filter((book) => {
      // Lọc thể loại: hỗ trợ cả category là object hoặc string
      const matchesGenre =
        (typeof book.category === "string" &&
          book.category.toLowerCase() === genre.toLowerCase()) ||
        (typeof book.category === "object" &&
          book.category?.name?.toLowerCase() === genre.toLowerCase());
      ...
      // Lọc giá: lấy newPrice nếu có, nếu không lấy price, nếu không có thì 0
  ...
  // DEBUG: log sortBy và giá từng book sau khi lọc
  ...
  const sortOptions = [
    { value: "newest", label: t("filter.newestFirst"), icon: RiCalendarLine },
    { value: "oldest", label: t("filter.oldestFirst"), icon: RiCalendarLine },
    {
      value: "price_asc",
    ...
    { value: "title", label: t("filter.titleAZ"), icon: RiBookOpenLine },
  ];
                    ...
                    >
                      ...
                      <div className="relative overflow-hidden h-64">
                        <img
                          src={
                            book.coverImage ||
                            "/placeholder.svg?height=256&width=192"
                          }
                          alt={book.title}
                        ...
                        <motion.div
                          ...
                          initial={{ opacity: 0, y: 20 }}
                          animate={{
                            opacity: hoveredBook === book._id ? 1 : 0,
                            y: hoveredBook === book._id ? 0 : 20,
                          }}
